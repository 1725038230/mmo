...
namespace GameServer.Services
{
    class UserService : Singleton<UserService>
    {
        ...
        public void OnGameEnter(NetConnection<NetSession> sender, UserGameEnterRequest request)
        {
            TCharacter dbchar = sender.Session.User.Player.Characters.ElementAt(request.characterIdx);
            Log.InfoFormat(" UserGameEnterRequest:characterIdx:id{0} {1},map:{2}", dbchar.ID, dbchar.Name, dbchar.MapID);
            Character character = CharacterManager.Instance.AddCharacter(dbchar);
            Log.InfoFormat("位置：{0}", character.Id);
            NetMessage message = new NetMessage();
            message.Response = new NetMessageResponse();
        ...
        public void OnGameLeave(NetConnection<NetSession> sender, UserGameLeaveRequest request)
        {
