...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        private void OnMapEntitySync(NetConnection<NetSession>sender,MapEntitySyncRequest request)
        {
            ...
            //Log.InfoFormat("OnMapEntitySync:characterID:{0};{1}  Entity.Id:{2} Event:{3} Entity:{4}", character.Id, character.Info.Name, request.entitySync.Id, request.entitySync.Event, request.entitySync.Entity.ToString());
        ...
        void OnMapTeleport(NetConnection<NetSession>sender,MapTeleportRequest request)
        {
            ...
            {
                Log.WarningFormat("Source TeleporterID [{0}] Link to ID[{1}] not existed", request.teleporterId,source.LinkTo);
            }
